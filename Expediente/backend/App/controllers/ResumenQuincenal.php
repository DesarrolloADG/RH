...
use \Core\MasterDom;
use \Core\Controller;
use \App\models\ResumenQuincenal as ResumenQuincenalDao;

class ResumenQuincenal extends Controller{
      ...
      <script>
          $(document).ready(function(){
            ...
            $("#btnAplicar").click(function(){
              $.ajax({
                url: '/ResumenQuincenal/getTablaRangoFechas',
                type: 'POST',
                ...
                success: function(response){
                  ...
                  $("#contenedor_tabla").html('<table class="table table-striped table-bordered table-hover table table-responsive" id="muestra-cupones" name="muestra-cupones"><thead><tr>'+datos['encabezado']+'</tr></thead><tbody>'+datos['tabla']+'</tbody></table>');
            ...
            $("#btnGuardar").click(function(){
              $.ajax({
                url: '/ResumenQuincenal/guardarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
              ...
              $.ajax({
                url: '/ResumenQuincenal/guardarPeriodoAsistencia',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
            ...
            $("#periodo_id").change(function(){
              $.ajax({
                url: '/ResumenQuincenal/verificarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
                  ...
                  if(response == 0){
                    ...
                    $("#alerta").html('<h3><strong>Atencion</strong> El periodo seleccionado esta abierto.</h3>');
                      ...
                      if(response == -1){
                        ...
                        $("#alerta").html('<h3><strong>Atencion</strong> El periodo seleccionado esta cerrado.</h3>');
                      }else{
                        if(response == -2){
                          $("#btnGuardar").hide();
                          $("#btnRestaurarPeriodo").hide();
            ...
            $("#btnCancelarPeriodo").click(function(){
              $.ajax({
                url: "/ResumenQuincenal/cancelarPeriodo",
                type: "POST",
            ...
            $("#btnRespaldarPeriodo").click(function(){
              $.ajax({
                url: '/ResumenQuincenal/respaldarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  $("#periodo_id").change();
            ...
            $("#btnRestaurarPeriodo").click(function(){
              $.ajax({
                url: '/ResumenQuincenal/restaurarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  $("#periodo_id").change();
...
html;
    $sPeriodo = '';
    $periodos = ResumenQuincenalDao::getPeriodos('QUINCENAL');
    if(count($periodos) == 0){
...
html;
    ...
    $userLogin = ResumenQuincenalDao::getDatosUsuarioLogeado($this->__usuario);
      ...
      View::render("resumen_quincenal");
    }

    public function respaldarPeriodo(){
      foreach (ResumenQuincenalDao::getPeriodoColaboradoresById(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenQuincenalDao::insertPeriodoBackup( (Object) $value);
        //echo $id.'-';
      }
    }

    public function restaurarPeriodo(){
      foreach (ResumenQuincenalDao::getPeriodoBackup(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenQuincenalDao::restaurarPeriodo( (Object) $value);

        //echo $id.'-';
      }
      $id = ResumenQuincenalDao::deletePeriodoBackup(MasterDom::getData('periodo_id'));
      $id = ResumenQuincenalDao::updatePeriodo(MasterDom::getData('periodo_id'));
    }

    public function cancelarPeriodo(){
      $id = ResumenQuincenalDao::cancelarPeriodo();
      print_r($id);
    }

    public function verificarPeriodo(){
        $periodo_id = MasterDom::getData('periodo_id');
        $periodo = ResumenQuincenalDao::getPeriodo($periodo_id);

        if($periodo['status'] == 0){
          if(count(ResumenQuincenalDao::getPeriodoBackup($periodo_id)) > 0){
            echo 1;
          }else{
            echo 0;
          }
        }elseif ($periodo['status'] == 1) {
          if($periodo_id == ResumenQuincenalDao::getMaxPeriodo()['id']){
            if(count(ResumenQuincenalDao::getPeriodoBackup($periodo_id)) > 0){
              echo -2;
            }else{
              echo -1;
    ...
    public function getTablaRangoFechas(){
      ...
      $periodo = ResumenQuincenalDao::getPeriodoById(MasterDom::getData('periodo_id'));
...
html;
      $j = 0;
      $administrador = ResumenQuincenalDao::getDatosUsuarioLogeado(MasterDom::getSesion('usuario'));
      /*if($administrador['perfil_id'] == 6){
        $colaboradores = ResumenQuincenalDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), '');
      }else{*/
        $colaboradores = ResumenQuincenalDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])),$administrador['administrador_id'],$administrador['usuario']);
      //}
...
html;
        ...
        $i=1;
        $horario_laboral = ResumenQuincenalDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
          ...
          foreach ($horario_laboral as $llave1 => $valor1) {
            if($dia_aux != $valor1['dia_semana']){
              ...
              if($dia_aux  == $dias_traductor[date('l', strtotime($fecha_inicio->format('Y-m-d')))]){
                  ...
                  $incidencia = ResumenQuincenalDao::getIncidencia($datos);
                  if(count($incidencia)>0){
                    ...
                    $datos->fecha_inicio = $fecha_inicio->format('Y-m-d').' '.ResumenQuincenal::restarMinutos($valor1['hora_entrada'],30);
                    $datos->fecha_fin = $fecha_inicio->format('Y-m-d').' '.ResumenQuincenal::sumarMinutos($valor1['hora_entrada'], intval($valor1['tolerancia_entrada']));
                    $registro_entrada = ResumenQuincenalDao::getAsistencia($datos);
...
html;
        ...
        $tabla .=<<<html
        <td>
          <a href="/AsignarIncentivos/incentivos/{$value['catalogo_colaboradores_id']}/{$periodo['prorrateo_periodo_id']}/ResumenQuincenal" class="btn btn-primary"><span  style="color:white"></span> Ver Incentivos</a>
        </td>
      </tr>
html;
        $j++;
...
html;
    ...
    public function guardarPeriodoAsistencia(){
      ...
      if($administrador['perfil_id'] == 6){
        ...
        $colaboradores = ResumenQuincenalDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), $administrador['administrador_id']);
      }
      foreach($colaboradores as $key => $value){
        ...
        while($fecha_inicio <= $fecha_fin){
          ...
          if($llegada != ''){
            ...
            $id = ResumenQuincenalDao::insertPeriodoAsistencia($asistencia);
            echo "Se ha insertado el id $id<br>";
          }
            $fecha_inicio->add(new \DateInterval('P1D'));
        }
      }
      ResumenQuincenalDao::updatePeriodo(MasterDom::getData('periodo_id'));
    }

    public function guardarPeriodo(){
        $periodo = ResumenQuincenalDao::getPeriodoById(MasterDom::getData('periodo_id'));
        //$periodo = ResumenQuincenalDao::getPeriodoById('3');
        $fecha_fin = new \DateTime($periodo['fecha_fin']);
        $dias_traductor = array(
            'Monday' => 'Lunes',
            'Tuesday' => 'Martes',
        ...
        foreach($colaboradores as $key => $value){
           ...
           /* OBTENCION DE DIAS LABORALES DEL COLABORADOR EN SUS HORARIOS */
            $dias_laborales = ResumenQuincenalDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
            $ultimo_horario = ResumenQuincenalDao::getLastHorario($value['catalogo_colaboradores_id']);
            $horarios = ResumenQuincenalDao::getListHorario($value['catalogo_colaboradores_id']);
                     ...
                     foreach($dias_laborales AS $llave => $valor){
                       if($dia_aux != $valor['dia_semana']){
                        ...
                        if($valor['dia_semana'] == $nombre_dia_semana){
                          ...
                          ResumenQuincenalDao::insertProrrateoColaboradorHorario($colaborador);
                        }
    ...
    public function generarExcel(){
      ...
      $controlador = "Resumen Quincenal";
      $columna = array('A','B','C','D','E','F','G','H','I','J','K');
      $nombreColumna = array('Numero Antiguedad','Nombre','Economicos');

      $objPHPExcel->getActiveSheet()->SetCellValue('A'.$fila, 'Reporte Quincenal de Periodos');
      ...
      $columna_enzabezado = 0;
      $resumen_semanal = ResumenQuincenalDao::getColaboradoresPeriodo(MasterDom::getData('periodo_id'));
      foreach ($resumen_semanal as $key => $value) {
        ...
        while ($fecha_inicio <= $fecha_fin){
          $datos->_fecha = $fecha_inicio->format('Y-m-d');
          $valor = ResumenQuincenalDao::getFechaPeriodo($datos);

          if($valor['estatus'] > 0){
            $valor['estatus'] = ResumenQuincenalDao::getIncidenciaById($valor['estatus'])['identificador_incidencia'];
      ...
      header('Content-Disposition: attachment;filename="Reporte Quincenal AG '.$controlador.'.xlsx"');
    ...
    public function getHorario($id){
      $dia = '';
      foreach (ResumenQuincenalDao::getHorarioLaboral($id) as $key => $value) {
