...
html;
            ...
            while($fecha_inicio <= $fecha_fin){
                    ...
                    if($dia_aux != $valor1['dia_semana']){
                        ...
                        if($dia_aux  == $dias_traductor[$fecha_inicio->format('l')]){
                            ...
                            if(count($incidencia)>0){
                                ...
                                $incidenciaFestivaDomingo = ResumenesDao::getIncidenciaFestivaDomingo($datos);

                                if(count($incidenciaFestivaDomingo)>0){
                                    $llegada = 'ADFD';
                                }
                                else
                                {
                                ...
                                if(count($registro_entrada) > 0){
                                    ...
                                    if(count(ResumenSemanalDao::getDiaFestivo($fecha_inicio->format('Y-m-d'))) >0){
                                        $llegada = 'ADF'; // asistencia en dia festivo (-24)
                                    }
                                    ...
                                    if(count($registro_entrada) > 0){
                                        ...
                                        if($value['privilegiado'] == 1){
                                            $incidenciaFestiva = ResumenesDao::getIncidenciaFestiva($datos);

                                            if(count($incidenciaFestiva)>0){
                                                $llegada = 'ADF';
                                            }
                                            else
                                            {
                ...
                if($llegada == ''){
                    ...
                    if(count($incidencia)>0){
                        ...
                        else
                        {
                            ...
                            if(!empty($cantidadDomingo))
                            {
                                if($incidencia[0]['identificador_incidencia'] == 'A')
                                {
                                    $llegada = 'ADD';
                                }
                            }
                        ...
                        if(count(ResumenSemanalDao::getDiaFestivo($fecha_inicio->format('Y-m-d'))) >0)
                        {
                            ...
                            if(count($registro_entrada)>0)
                            {
                                ...
                                $llegada = 'ADF'; // asistencia en dia festivo (-24)
                                if(count(ResumenSemanalDao::getDiaFestivoDomingo($fecha_inicio->format('Y-m-d'))) >0)
                                {
                                    ///////
                                    $llegada = 'ADFD'; // asistencia en dia festivo (-24)
                        ...
                        else
                        {
                            if(count($registro_entrada)>0)
                            {
                                $llegada = 'ADD'; // asistencia (0) en dia  no laboral
                            }
                        }
                ...
                if ($llegada == 'A' || $llegada =='ADD'){$color = 'green';}
                elseif($llegada == 'D' || $llegada == 'DF' || $llegada == 'FDF' ){$color = 'gray';}
                elseif($llegada == 'R' || $llegada == 'RDF'){$color = 'yellow';}
                elseif ($llegada == 'FF' || $llegada == 'F'){$color = 'red';}
                elseif ($llegada == 'ADFD' || $llegada == 'ADFD'){$color = 'orange';}
                elseif ($llegada == 'ADF' || $llegada == 'ADF'){$color = 'blue';}
...
html;
            ...
            $cantidadDomingo = $this->getValoresDomingos($value['catalogo_colaboradores_id'], $idPeriodo);
            $botesUsuario = $this->getIncentivoBotes($value['catalogo_colaboradores_id'], $idPeriodo);
            $tabla.=<<<html
...
html;
    ...
    public function getIncentivoBotes($colaboradorId, $prorrateoId){
        $data = new \stdClass();
        $data->_colaborador_id = $colaboradorId;
        $data->_prorrateo_periodo_id = $prorrateoId;
    ...
    public function getBotes($colaborador_id, $prorrateo_periodo_id){
    ...
    public function getValoresDomingos($colaborador_id, $id_periodo){
        ...
        $val = 0;
        if($result['procesos'] != $result['laborado'])
        {
            if($result['procesos'] != 0)
                $val = $result['procesos'];
            if($result['laborado'] != 0)
                $val = $result['laborado'];
        }
        else
        {
    ...
    public function setTablaExistente($idPeriodo, $tipoPeriodo, $perfil_id, $catalogo_planta_id, $catalogo_departamento_id, $catalodo_planta_nombre, $accion, $post){
        ...
        $fecha_inicio = new \DateTime($periodo['fecha_inicio']);
        $fecha_fin = new \DateTime($periodo['fecha_fin']);
        $convertir_registro = array( 0 => "A", -1 => "F", -11 => "FF" , -2 => "R" , -22 => "FR" , -23 => "DF", -24 => "ADD" , -25 => "RDF" , -26 => "FRDF" );
            ...
            while ($fechaTest <=$fecha_fin) {
                ...
                foreach ($registro as $keyRegistro => $valueRegistro) {
                    if($valueRegistro['fecha'] == $fechaTest->format('Y-m-d')){
                        ...
                        //////////////
                        $datos1 = new \stdClass();
                        $datos1->numero_empleado = $value['numero_empleado'];
                        $fecha_inicio = new \DateTime($valueRegistro['fecha']);

                        $datos1->_catalogo_colaboradores_id = $value['catalogo_colaboradores_id'];
                        $datos1->_fecha = $fecha_inicio->format('Y-m-d');
                        $incidenciaFestivaDomingo = ResumenesDao::getIncidenciaFestivaDomingo($datos1);
                        //////////////
                        if($llegada == ''){
...
html;
    ...
    public function propiosSemanales(){
        ...
        if($user['perfil_id'] == 6 || $user['perfil_id'] == 5){ // Si el usuario es de RH
...
html;
            ...
            while($fecha_inicio <= $fecha_fin){
                ...
                elseif($llegada == 'FDF'){$color = 'gray';}
                elseif($llegada == 'R'){$color = 'yellow';}
                elseif ($llegada == 'FF'){$color = 'red';}
                elseif ($llegada == 'ADFD'){$color = 'orange';}
                elseif ($llegada == 'ADF' ){$color = 'blue';}
                elseif ($llegada == 'FR'){$color = 'orange';}
                else{$color = 'green';}
