...
use \Core\MasterDom;
use \Core\Controller;
use \App\models\ResumenSemanal as ResumenDao;
      ...
      <script>
          $(document).ready(function(){
            ...
            $("#btnAplicar").click(function(){
              $.ajax({
                url: '/Resumen/getTablaRangoFechas',
                type: 'POST',
            ...
            $("#btnGuardar").click(function(){
              ...
              $.ajax({
                url: '/Resumen/guardarPeriodoAsistencia',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
...
html;
    $sPeriodo = '';
    $periodos = ResumenDao::getPeriodos('SEMANAL');
    if(count($periodos) == 0){
...
html;
    ...
    public function respaldarPeriodo(){
      foreach (ResumenDao::getPeriodoColaboradoresById(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenDao::insertPeriodoBackup( (Object) $value);
        echo $id.'-';
      }
    }

    public function restaurarPeriodo(){
      foreach (ResumenDao::getPeriodoBackup(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenDao::restaurarPeriodo( (Object) $value);

        echo $id.'-';
      }
      $id = ResumenDao::deletePeriodoBackup(MasterDom::getData('periodo_id'));
      $id = ResumenDao::updatePeriodo(MasterDom::getData('periodo_id'));
    }

    public function cancelarPeriodo(){
      $id = ResumenDao::cancelarPeriodo();
      print_r($id);
    }

    public function verificarPeriodo(){
        $periodo_id = MasterDom::getData('periodo_id');
        $periodo = ResumenDao::getPeriodo($periodo_id);

        if($periodo['status'] == 0){
          if(count(ResumenDao::getPeriodoBackup($periodo_id)) > 0){
            echo 1;
          }else{
            echo 0;
          }
        }elseif ($periodo['status'] == 1) {
          if($periodo_id == ResumenDao::getMaxPeriodo()['id']){
            if(count(ResumenDao::getPeriodoBackup($periodo_id)) > 0){
              echo -2;
            }else{
              echo -1;
    ...
    public function getTablaRangoFechas(){
      ...
      $periodo = ResumenDao::getPeriodoById(MasterDom::getData('periodo_id'));
...
html;
      $j = 0;
      $administrador = ResumenDao::getDatosUsuarioLogeado(MasterDom::getSesion('usuario'));
      foreach(ResumenDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), $administrador['administrador_id']) as $key => $value){
        $tabla .=<<<html
        <tr>
...
html;
        ...
        $i=1;
        $horario_laboral = ResumenDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
          ...
          foreach ($horario_laboral as $llave1 => $valor1) {
            ...
            if($dia_aux != $valor1['dia_semana']){
              ...
              if($dia_aux  == $dias_traductor[date('l', strtotime($fecha_inicio->format('Y-m-d')))]){
                  ...
                  $incidencia = ResumenDao::getIncidencia($datos);
                  if(count($incidencia)>0){
                    ...
                    $datos->fecha_inicio = $fecha_inicio->format('Y-m-d').' '.Resumen::restarMinutos($valor1['hora_entrada'],30);
                    $datos->fecha_fin = $fecha_inicio->format('Y-m-d').' '.Resumen::sumarMinutos($valor1['hora_entrada'], intval($valor1['tolerancia_entrada']));
                    $registro_entrada = ResumenDao::getAsistencia($datos);
...
html;
        ...
        $tabla .=<<<html
        <td>
          <a href="/AsignarIncentivos/incentivos/{$value['catalogo_colaboradores_id']}/{$periodo['prorrateo_periodo_id']}" class="btn btn-primary"><span  style="color:white"></span> Ver Incentivos </a>
        </td>
      </tr>
html;
        $j++;
...
html;
    ...
    public function guardarPeriodo(){
        ...
        foreach(ResumenDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), $administrador['administrador_id']) as $key => $value){
           ...
           /* OBTENCION DE DIAS LABORALES DEL COLABORADOR EN SUS HORARIOS */
            $dias_laborales = ResumenDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
            $ultimo_horario = ResumenDao::getLastHorario($value['catalogo_colaboradores_id']);
            $horarios = ResumenDao::getListHorario($value['catalogo_colaboradores_id']);
                   ...
                   foreach($dias_laborales AS $llave => $valor){
                     if($dia_aux != $valor['dia_semana']){
                      ...
                      if($valor['dia_semana'] == $nombre_dia_semana){
                              ...
                              //ResumenDao::insertProrrateoColaboradorHorario($colaborador);
                         }
                     ...
                     if($catalogo_horario_id != '' && $value['horario_tipo'] == 'semanal'){
    ...
    public function generarExcel(){
      ...
      $columna_enzabezado = 0;
      $resumen_semanal = ResumenDao::getColaboradoresPeriodo(MasterDom::getData('periodo_id'));
      foreach ($resumen_semanal as $key => $value) {
        ...
        while ($fecha_inicio <= $fecha_fin){
          $datos->_fecha = $fecha_inicio->format('Y-m-d');
          $valor = ResumenDao::getFechaPeriodo($datos);

          if($valor['estatus'] > 0){
            $valor['estatus'] = ResumenDao::getIncidenciaById($valor['estatus'])['identificador_incidencia'];
    ...
    public function getHorario($id){
      $dia = '';
      foreach (ResumenDao::getHorarioLaboral($id) as $key => $value) {
