...
class Resumenes_bk extends Controller{
  ...
  public function setTabla($idPeriodo, $tipoPeriodo, $perfil_id, $catalogo_planta_id, $catalogo_departamento_id, $catalodo_planta_nombre, $accion){
...
html;
    $j = 0;
    $colaboradores = ResumenesDao::getAllColaboradores($perfil_id, $tipoPeriodo, $catalogo_planta_id, $catalogo_departamento_id, $catalodo_planta_nombre, $accion);
    ...
    <td>$nombreEmpleado</td>
          <td>
            <b>Economicos</b> {$value['nombre_departamento']} <br>
...
/*
        if($value['catalogo_colaboradores_id'] == 124){
          echo 'Ultimo horario: ';
          print_r($ultimo_horario);
          echo '<br>Horarios: ';
          print_r($horarios);
        }
*/
...
html;
        if($tipoPeriodo == "Semanal"){
          $tabla .=<<<html
            ...
            <a href="/Incentivo/incentivos/{$value['catalogo_colaboradores_id']}/{$periodo['prorrateo_periodo_id']}/semanales/" target="_blank" class="btn btn-primary" data-toggle="tooltip" title="VER INCENTIVOS">
...
html;
  ...
  public function setTablaExistente($idPeriodo, $tipoPeriodo, $perfil_id, $catalogo_planta_id, $catalogo_departamento_id, $catalodo_planta_nombre, $accion){
    $periodo = ResumenesDao::getPeriodoById($idPeriodo);
    $hidden = ($periodo['status'] == 0)? '': ' hidden ';
    $fecha_inicio = new \DateTime($periodo['fecha_inicio']);
    $fecha_fin = new \DateTime($periodo['fecha_fin']);
    $convertir_registro = array(0=>"A",-1 => "F",-2 => "R",-22 => "FR",-23 => "DF",-24 => "AA",-25 => "RDF",-26 => "FRDF");
...
html;
    ...
    foreach(ResumenesDao::getAllColaboradores($perfil_id, $tipoPeriodo, $catalogo_planta_id, $catalogo_departamento_id, $catalodo_planta_nombre, $accion) as $key => $value){
    ...
    $extraFooter = <<<html
        ...
        $(document).ready(function(){
          ...
          var oTable = $('#muestra-colaboradores').DataTable({
                  ...
                  $(".buttons-csv").addClass('btn btn-success btn-sm');
                  $(".buttons-pdf").addClass('btn btn-success btn-sm');
          ...
          $("#btnAplicar").click(function(){
            ...
            $("#btnGuardar").click(function(){
              ...
              //alert("Periodo: "+$("#periodo_id").val()+" Tipo_periodo: "+$("#tipo_periodo").val()+" Mensaje: "+$("#mensaje").val());
...
html;
  ...
  public function propiosSemanales(){
    ...
    if($user['perfil_id'] == 6){ // Si el usuario es de RH
      $tituloVista = "Incidencias de Recursos Humanos - Vacaciones " . strtolower($user['nombre_planta']) . " - Depto. " . $user['nombre'];
      //$val = 3; // TIENE INCENTIVOS PROPIOS
      $val = ($user['catalogo_planta_id'] == 1) ? 1 : 1;
    }

    if($user['perfil_id'] == 1){ // Si el usuario es de RH
      $tituloVista = "Incidencias de ROOT - Vacaciones " . strtolower($user['nombre_planta']) . " - Depto. " . $user['nombre'];
      $val = 3; // TIENE INCENTIVOS PROPIOS
    }
    ...
    echo $this->setTabla($periodo_id, "Semanal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $val);
  ...
  public function propiosQuincenales(){
    ...
    $this->setTabla($periodo_id, "Quincenal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $accion /*propiosSemanales*/); 
    $extraFooter = $this->getFooterTable();
  ...
  public function propiosSemanalesHistoricos(){
    ...
    echo $this->setTablaExistente($idPeriodo, "Semanal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $accion /*propiosSemanales*/); 
  ...
  public function propiosQuincenalesHistoricos(){
    ...
    echo $this->setTablaExistente($idPeriodo, "Quincenal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $accion /*propiosSemanales*/); 
  ...
  public function semanales(){
    ...
    echo $this->setTabla($periodo_id, "Semanal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $accion /*propiosSemanales*/);
  ...
  public function quincenales(){
    ...
    echo $this->setTabla($periodo_id, "Quincenal", $user['perfil_id'], $user['catalogo_planta_id'], $user['catalogo_departamento_id'], $user['nombre_planta'], $accion /*propiosSemanales*/); 
...
html;
      ...
      $i++;
            ...
            $tabla .=<<<html
          <td>
            <a href="/Incentivo/incentivos/{$value['catalogo_colaboradores_id']}/{$periodo['prorrateo_periodo_id']}/semanales" class="btn btn-primary"><span  style="color:white"></span> Ver Incentivos</a>
          </td>
        </tr>
html;
    $j++;
