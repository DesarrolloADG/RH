...
use \Core\MasterDom;
use \Core\Controller;
use \App\models\ResumenSemanalRH as ResumenQuincenalRHDao;

class ResumenQuincenalRH extends Controller{
      ...
      <script>
          $(document).ready(function(){
            ...
            $("#btnAplicar").click(function(){
              $.ajax({
                url: '/ResumenQuincenalRH/getTablaRangoFechas',
                type: 'POST',
            ...
            $("#btnGuardar").click(function(){
              ...
              $.ajax({
                url: '/ResumenQuincenalRH/guardarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
              ...
              $.ajax({
                url: '/ResumenQuincenalRH/guardarPeriodoAsistencia',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
            ...
            $("#periodo_id").change(function(){
              $.ajax({
                url: '/ResumenQuincenalRH/verificarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  //alert(response);
            ...
            $("#btnCancelarPeriodo").click(function(){
              $.ajax({
                url: "/ResumenQuincenalRH/cancelarPeriodo",
                type: "POST",
            ...
            $("#btnRespaldarPeriodo").click(function(){
              $.ajax({
                url: '/ResumenQuincenalRH/respaldarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  $("#periodo_id").change();
            ...
            $("#btnRestaurarPeriodo").click(function(){
              $.ajax({
                url: '/ResumenQuincenalRH/restaurarPeriodo',
                type: 'POST',
                data:{periodo_id: $("#periodo_id").val()},
                success: function(response){
                  $("#periodo_id").change();
...
html;
    $sPeriodo = '';
    $periodos = ResumenQuincenalRHDao::getPeriodos('QUINCENAL');
    if(count($periodos) == 0){
...
html;
    ...
    public function respaldarPeriodo(){
      foreach (ResumenQuincenalRHDao::getPeriodoColaboradoresById(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenQuincenalRHDao::insertPeriodoBackup( (Object) $value);
        echo $id.'-';
      }
    }

    public function restaurarPeriodo(){
      foreach (ResumenQuincenalRHDao::getPeriodoBackup(MasterDom::getData('periodo_id')) as $key => $value) {
        $id = ResumenQuincenalRHDao::restaurarPeriodo( (Object) $value);

        echo $id.'-';
      }
      $id = ResumenQuincenalRHDao::deletePeriodoBackup(MasterDom::getData('periodo_id'));
      $id = ResumenQuincenalRHDao::updatePeriodo(MasterDom::getData('periodo_id'));
    }

    public function cancelarPeriodo(){
      $id = ResumenQuincenalRHDao::cancelarPeriodo();
      
    }

    public function verificarPeriodo(){
        $periodo_id = MasterDom::getData('periodo_id');
        $periodo = ResumenQuincenalRHDao::getPeriodo($periodo_id);

        if($periodo['status'] == 0){
          if(count(ResumenQuincenalRHDao::getPeriodoBackup($periodo_id)) > 0){
            echo 1;
          }else{
            echo 0;
          }
        }elseif ($periodo['status'] == 1) {
          if($periodo_id == ResumenQuincenalRHDao::getMaxPeriodo()['id']){
            if(count(ResumenQuincenalRHDao::getPeriodoBackup($periodo_id)) > 0){
              echo -2;
            }else{
              echo -1;
    ...
    public function getTablaRangoFechas(){
      ...
      $periodo = ResumenQuincenalRHDao::getPeriodoById(MasterDom::getData('periodo_id'));
...
html;
      $j = 0;
      $administrador = ResumenQuincenalRHDao::getDatosUsuarioLogeado(MasterDom::getSesion('usuario'));

        $colaboradores = ResumenQuincenalRHDao::getAllColaboradoresPago('Quincenal', $administrador['administrador_id'], $administrador['usuario'], $administrador['nombre_planta'] );
...
html;
        ...
        $i=1;
        $horario_laboral = ResumenQuincenalRHDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
          ...
          foreach ($horario_laboral as $llave1 => $valor1) {
            if($dia_aux != $valor1['dia_semana']){
              ...
              if($dia_aux  == $dias_traductor[date('l', strtotime($fecha_inicio->format('Y-m-d')))]){
                  ...
                  $incidencia = ResumenQuincenalRHDao::getIncidencia($datos);
                  if(count($incidencia)>0){
                    ...
                    $datos->fecha_inicio = $fecha_inicio->format('Y-m-d').' '.ResumenQuincenalRH::restarMinutos($valor1['hora_entrada'],30);
                    $datos->fecha_fin = $fecha_inicio->format('Y-m-d').' '.ResumenQuincenalRH::sumarMinutos($valor1['hora_entrada'], intval($valor1['tolerancia_entrada']));
                    $registro_entrada = ResumenQuincenalRHDao::getAsistencia($datos);
...
html;
        ...
        $tabla .=<<<html
        <td>
          <a href="/AsignarIncentivos/incentivos/{$value['catalogo_colaboradores_id']}/{$periodo['prorrateo_periodo_id']}/ResumenQuincenalRH" class="btn btn-primary"><span  style="color:white"></span> Ver Incentivos</a>
        </td>
      </tr>
html;
        $j++;
...
html;
    ...
    public function guardarPeriodoAsistencia(){
      ...
      if($administrador['perfil_id'] == 6){
        $colaboradores = ResumenQuincenalRHDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), '');
      }else{
        $colaboradores = ResumenQuincenalRHDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])), $administrador['administrador_id']);
      }
      ...
      foreach($colaboradores as $key => $value){
        ...
        while($fecha_inicio <= $fecha_fin){
          ...
          if($llegada != ''){
            ...
            $id = ResumenQuincenalRHDao::insertPeriodoAsistencia($asistencia);
            echo "Se ha insertado el id $id<br>";
          }
            $fecha_inicio->add(new \DateInterval('P1D'));
        }
      }
      ResumenQuincenalRHDao::updatePeriodo(MasterDom::getData('periodo_id'));
    }

    public function guardarPeriodo(){
        $periodo = ResumenQuincenalRHDao::getPeriodoById(MasterDom::getData('periodo_id'));
        //$periodo = ResumenQuincenalRHDao::getPeriodoById('3');
        $fecha_fin = new \DateTime($periodo['fecha_fin']);
        $dias_traductor = array(
            'Monday' => 'Lunes',
            'Tuesday' => 'Martes',
        ...
        foreach($colaboradores as $key => $value){
           ...
           /* OBTENCION DE DIAS LABORALES DEL COLABORADOR EN SUS HORARIOS */
            $dias_laborales = ResumenQuincenalRHDao::getHorarioLaboral($value['catalogo_colaboradores_id']);
            $ultimo_horario = ResumenQuincenalRHDao::getLastHorario($value['catalogo_colaboradores_id']);
            $horarios = ResumenQuincenalRHDao::getListHorario($value['catalogo_colaboradores_id']);
                     ...
                     foreach($dias_laborales AS $llave => $valor){
                       if($dia_aux != $valor['dia_semana']){
                        ...
                        if($valor['dia_semana'] == $nombre_dia_semana){
                          ...
                          ResumenQuincenalRHDao::insertProrrateoColaboradorHorario($colaborador);
                        }

                      }
                    }
                  }elseif($value['horario_tipo'] == 'Quincenal'){
                     /*************************************************************************************************/
                     $nombre_dia_semana = $dias_traductor[date('l',strtotime($fecha_inicio->format('Y-m-d')))];
    ...
    public function generarExcel(){
      ...
      $columna_enzabezado = 0;
      $resumen_Quincenal = ResumenQuincenalRHDao::getColaboradoresPeriodo(MasterDom::getData('periodo_id'));
      foreach ($resumen_Quincenal as $key => $value) {
        $columna_enzabezado = 0;
        ...
        while ($fecha_inicio <= $fecha_fin){
          $datos->_fecha = $fecha_inicio->format('Y-m-d');
          $valor = ResumenQuincenalRHDao::getFechaPeriodo($datos);

          if($valor['estatus'] > 0){
            $valor['estatus'] = ResumenQuincenalRHDao::getIncidenciaById($valor['estatus'])['identificador_incidencia'];
    ...
    public function getHorario($id){
      $dia = '';
      foreach (ResumenQuincenalRHDao::getHorarioLaboral($id) as $key => $value) {
