        ...
        $(document).ready(function(){
          ...
          var oTable = $('#muestra-cupones').DataTable({
            ...
            $("#export_pdf").click(function(){
              $('#all').attr('action', '/Vacaciones/generarPDF/');
              $('#all').attr('target', '_blank');
              $("#all").submit();
            });

            $("#export_excel").click(function(){
              $('#all').attr('action', '/Vacaciones/generarExcel/');
              $('#all').attr('target', '_blank');
              $("#all").submit();
            });
            ...
            $("#delete").click(function(){
              ...
              if(seleccionados>0){
                alertify.confirm('¿Segúro que desea eliminar lo seleccionado?', function(response){
                  if(response){
                    $('#all').attr('action', '/Vacaciones/delete');
                    $('#all').attr('target', '');
                    $("#all").submit();
      ...
      foreach ($plantas as $key => $value) {
        $tabla.=<<<html
                <tr>
                    ...
                    <td>{$value['status']}</td>
                    <td class="center" >
                        <a href="/Vacaciones/show/{$value['catalogo_planta_id']}" type="submit" name="id_planta" class="btn btn-success"><span class="glyphicon glyphicon-eye-open" style="color:white"></span> </a>
                    </td>
                </tr>
html;
      }
...
html;
      ...
      exit;
      //$ids = MasterDom::getDataAll('borrar');
      //echo shell_exec('php -f /home/granja/backend/public/librerias/mpdf_apis/Api.php Vacaciones '.json_encode(MasterDom::getDataAll('borrar')));
    }
