...
sql;
    ...
    public static function getDia($datos1){
        $mysqli = Database::getInstance();
        $query=<<<sql
        SELECT (ELT(WEEKDAY(fecha) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS DIA_SEMANA
        FROM prorrateo_periodo_colaboradores WHERE prorrateo_periodo_id = 247
        AND catalogo_colaboradores_id =  $datos1->catalogo_colaboradores_id 
sql;
        return $mysqli->queryAll($query);
    }
...
sql;
    ...
    public static function getIncidenciaFestiva($datos){
        $mysqli = Database::getInstance();
        $query =<<<sql
        SELECT ci.* 
FROM catalogo_incidencia ci
JOIN prorrateo_colaboradores_incidencia pci ON 
pci.catalogo_incidencia_id = ci.catalogo_incidencia_id
INNER JOIN catalogo_dia_festivo cdf ON cdf.fecha = pci.fecha_incidencia
WHERE pci.catalogo_colaboradores_id = $datos->_catalogo_colaboradores_id AND pci.fecha_incidencia = '$datos->_fecha' AND cdf.status=1 AND Identificador_incidencia = 'A'
sql;
        return $mysqli->queryAll($query);
    }

    public static function getIncidenciaFestivaDomingo($datos){
        $mysqli = Database::getInstance();
        $query =<<<sql
        SELECT ci.*, cdfv.dia_semana 
FROM catalogo_incidencia ci
JOIN prorrateo_colaboradores_incidencia pci ON 
pci.catalogo_incidencia_id = ci.catalogo_incidencia_id
INNER JOIN catalogo_dia_festivo cdf ON cdf.fecha = pci.fecha_incidencia
INNER JOIN catalogo_dia_festivo_view cdfv ON cdfv.fecha = cdf.fecha
WHERE pci.catalogo_colaboradores_id = $datos->_catalogo_colaboradores_id AND pci.fecha_incidencia = '$datos->_fecha' AND cdf.status=1 AND ci.Identificador_incidencia = 'A'  AND cdfv.DIA_SEMANA = 'Domingo'
sql;
        return $mysqli->queryAll($query);
    }
}
