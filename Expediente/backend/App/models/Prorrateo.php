...
AND cc.identificador_noi like "$identificador" GROUP BY fecha
sql;
...
sql;
    ...
    public static function verificaAsistenciaFestiva($prorrateo_periodo_id, $catalogo_colaboradores_id, $fecha){
      $mysqli = Database::getInstance();
      $query = <<<sql
SELECT  COUNT(*) AS asistencia FROM prorrateo_periodo_colaboradores AS ppc 
JOIN catalogo_dia_festivo_view cdfv ON cdfv.fecha = ppc.fecha 
WHERE ppc.prorrateo_periodo_id = $prorrateo_periodo_id
AND ppc.catalogo_colaboradores_id =  $catalogo_colaboradores_id AND ppc.fecha =  "$fecha" AND cdfv.DIA_SEMANA != 'Domingo' AND ppc.estatus !='18' AND ppc.estatus !='19' AND ppc.estatus !='3' AND ppc.estatus !='22' AND ppc.estatus !='8'
sql;
      return $mysqli->queryOne($query);
    }
    public static function verificaAsistenciaFestivaDomingo($prorrateo_periodo_id, $catalogo_colaboradores_id, $fecha){
        $mysqli = Database::getInstance();
        $query = <<<sql
SELECT count(*) AS asistencia FROM prorrateo_periodo_colaboradores WHERE prorrateo_periodo_id = $prorrateo_periodo_id AND catalogo_colaboradores_id = $catalogo_colaboradores_id AND fecha = "$fecha" and estatus = '24'
sql;
        return $mysqli->queryOne($query);
    }
}
